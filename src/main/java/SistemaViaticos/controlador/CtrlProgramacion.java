 ...
 */
package SistemaViaticos.controlador;

import SistemaViaticos.modelo.Autorizacion;
import SistemaViaticos.modelo.CajaMenor;
import SistemaViaticos.modelo.ConsultaEmpleados;
import SistemaViaticos.modelo.CostoViatico;
import SistemaViaticos.modelo.Detalle_viatico;
import SistemaViaticos.modelo.Municipio;
import SistemaViaticos.modelo.Empleado;
import SistemaViaticos.modelo.Viatico;
import SistemaViaticos.dialogs.diaEditViatico;
...
import SistemaViaticos.paneles.IconCellRenderer;
import SistemaViaticos.paneles.pnlProgramacion;
import SistemaViaticos.paneles.Caja_menor;
/**
 *
 * @author AUXINVTIC
 */
public class CtrlProgramacion{
    ...
    public void registrarViatico(pnlProgramacion pnl){ 
     ...
     public void registrarCaja_menor(Caja_menor pnl){
        SimpleDateFormat sdfDB = new SimpleDateFormat("yyyy-MM-dd");
        String  Codigo_Cuenta = pnl.cmbCuenta.getSelectedItem().toString();
        String  Codigo_Subcuenta = pnl.cmbSubCuenta.getSelectedItem().toString();
        String  Codigo_Talonario = pnl.cmbTalonario.getSelectedItem().toString();
        String  Fecha = sdfDB.format(pnl.fechaInicial.getDate());
        String  txtPersona = pnl.txtPersona.getText();
        String  txtDetalle = pnl.txtDetalle.getText();
        String  txtCantidad = pnl.txtCantidad.getText();
        String  txtValorUnitario = pnl.txtValorUnitario.getText();
        String  txtValorTotal = pnl.txtValorTotal.getText();
        ...
        String hoActual = h.format(hora);*/
        /*
        Date fechaInicial = pnl.fechaInicial1.getDate();
        

        SimpleDateFormat fi = new SimpleDateFormat(strDateFormat);
        String fechainicial = fi.format(fechaInicial);
        */
       
        //int dias=(int) ((fechaFinal.getTime()-fechaInicial.getTime())/86400000);       
        CajaMenor caja_menor = new CajaMenor();
        caja_menor.setCodigo_Cuenta(Codigo_Cuenta);
        caja_menor.setCodigo_Subcuenta(Codigo_Subcuenta);        
        caja_menor.setCodigo_Talonario(Codigo_Talonario);
        caja_menor.setFecha(Fecha);
        caja_menor.setPersona_Servicio(txtPersona);
        caja_menor.setDetalle(txtDetalle);
        caja_menor.setCantidad(Integer.parseInt(txtCantidad));
        caja_menor.setValor_Unitario(Double.parseDouble(txtValorUnitario));
        caja_menor.setValor_Total(Double.parseDouble(txtValorTotal));
        /*if(tecnico.getId_municipio().equals("01")){
            viatico.setEstado("1");
        }else{
            viatico.setEstado("5");
        ...
        viatico.setUsuario(pnl.txtIdUsuario.getText());  */      
        //JOptionPane.showMessageDialog(null, dias);
           modelo.registroCaja_menor(caja_menor);
        /*if(modelo.registroViaticos(viatico)){
            int id_viatico = modelo.ultimo_id_viatico();            
            for(int i=0; i<pnl.tablaCostoViaticos.getRowCount(); i++){
                if(IsSelected(i, 3, pnl.tablaCostoViaticos)){
